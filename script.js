...
async function firstLoadInit() {
    // PWA mode: skip CSRF token (no backend server)
    try {
        const tokenResponse = await fetch('/csrf-token');
        const tokenData = await tokenResponse.json();
        token = tokenData.token;
    } catch {
        // PWA: no backend, use empty token
        token = '';
        console.log('[PWA] No CSRF token (running without backend)');
    }
    ...
    // PWA: Wrap each initialization step in individual try-catch so one failure doesn't block the rest
    const pwaSafe = async (label, fn) => { try { await fn(); } catch (e) { console.warn('[PWA] Init step failed:', label, e); } };

    await pwaSafe('registerPromptManagerMigration', () => registerPromptManagerMigration());
    await pwaSafe('initDomHandlers', () => initDomHandlers());
    await pwaSafe('initStandaloneMode', () => initStandaloneMode());
    await pwaSafe('initLibraryShims', () => initLibraryShims());
    await pwaSafe('addShowdownPatch', () => addShowdownPatch(showdown));
    await pwaSafe('addDOMPurifyHooks', () => addDOMPurifyHooks());
    await pwaSafe('reloadMarkdownProcessor', () => reloadMarkdownProcessor());
    await pwaSafe('applyBrowserFixes', () => applyBrowserFixes());
    await pwaSafe('getClientVersion', () => getClientVersion());
    await pwaSafe('initSecrets', () => initSecrets());
    await pwaSafe('readSecretState', () => readSecretState());
    await pwaSafe('initLocales', () => initLocales());
    await pwaSafe('initChatUtilities', () => initChatUtilities());
    await pwaSafe('initDefaultSlashCommands', () => initDefaultSlashCommands());
    await pwaSafe('initTextGenModels', () => initTextGenModels());
    await pwaSafe('initOpenAI', () => initOpenAI());
    await pwaSafe('initTextGenSettings', () => initTextGenSettings());
    await pwaSafe('initKoboldSettings', () => initKoboldSettings());
    await pwaSafe('initNovelAISettings', () => initNovelAISettings());
    await pwaSafe('initSystemPrompts', () => initSystemPrompts());
    await pwaSafe('initExtensions', () => initExtensions());
    await pwaSafe('initExtensionSlashCommands', () => initExtensionSlashCommands());
    await pwaSafe('initToolSlashCommands', () => ToolManager.initToolSlashCommands());
    await pwaSafe('initPresetManager', () => initPresetManager());
    await pwaSafe('initSystemMessages', () => initSystemMessages());
    await pwaSafe('getSettings', () => getSettings(initLoaderHandle));
    await pwaSafe('checkOpenRouterAuth', () => checkOpenRouterAuth());
    await pwaSafe('initKeyboard', () => initKeyboard());
    await pwaSafe('initDynamicStyles', () => initDynamicStyles());
    await pwaSafe('initTags', () => initTags());
    await pwaSafe('initBookmarks', () => initBookmarks());
    await pwaSafe('getUserAvatars', () => getUserAvatars(true, user_avatar));
    await pwaSafe('getCharacters', () => getCharacters());
    await pwaSafe('getBackgrounds', () => getBackgrounds());
    await pwaSafe('initTokenizers', () => initTokenizers());
    await pwaSafe('initBackgrounds', () => initBackgrounds());
    await pwaSafe('initAuthorsNote', () => initAuthorsNote());
    await pwaSafe('initPersonas', () => initPersonas());
    await pwaSafe('initSlashCommandAutoComplete', () => initSlashCommandAutoComplete());
    await pwaSafe('initMacroAutoComplete', () => initMacroAutoComplete());
    await pwaSafe('initWorldInfo', () => initWorldInfo());
    await pwaSafe('initHorde', () => initHorde());
    await pwaSafe('initRossMods', () => initRossMods());
    await pwaSafe('initStats', () => initStats());
    await pwaSafe('initCfg', () => initCfg());
    await pwaSafe('initLogprobs', () => initLogprobs());
    await pwaSafe('initInputMarkdown', () => initInputMarkdown());
    await pwaSafe('initServerHistory', () => initServerHistory());
    await pwaSafe('initSettingsSearch', () => initSettingsSearch());
    await pwaSafe('initBulkEdit', () => initBulkEdit());
    await pwaSafe('initReasoning', () => initReasoning());
    await pwaSafe('initWelcomeScreen', () => initWelcomeScreen());
    await pwaSafe('initScrapers', () => initScrapers());
    await pwaSafe('initCustomSelectedSamplers', () => initCustomSelectedSamplers());
    await pwaSafe('initDataMaid', () => initDataMaid());
    await pwaSafe('initItemizedPrompts', () => initItemizedPrompts());
    await pwaSafe('initAccessibility', () => initAccessibility());
    await pwaSafe('initSwipePicker', () => initSwipePicker());
    await pwaSafe('addDebugFunctions', () => addDebugFunctions());
    await pwaSafe('doDailyExtensionUpdatesCheck', () => doDailyExtensionUpdatesCheck());
    await pwaSafe('APP_INITIALIZED', () => eventSource.emit(event_types.APP_INITIALIZED));
    await initLoaderHandle.hide();
    await fixViewport();
    await pwaSafe('APP_READY', () => eventSource.emit(event_types.APP_READY));
}
